...
The Community App package is also `0.14.0-dev.1`, pinned to the immutable multi-architecture GHCR digest `sha256:ca7e5d0bc440b70d2f3d01869a7aa7bae96cf5290bdf4fd70391dd5a4b9b79c0`.
...
- explicit connectivity states that distinguish a truly offline phone from an unavailable Umbrel/LAN/VPN/Tailscale path;
...
The shared shell probes same-origin `/healthz` and shows separate states for **`Телефон офлайн.`** and **`Umbrel недоступен.`**. This makes a broken local/VPN/Tailscale route visible even when the phone still has internet access.
...
`0.13.0-dev.1` added conservative `.3mf` → local price matching, economics at the actual customer price, client-safe quote copying, immutable post-print electricity reconciliation and snapshot-only price-discipline analytics.

`0.14.0-dev.1` is now the current Community App package. It adds the safe iPhone/Home Screen shell, no-store dynamic business responses and explicit connectivity-state handling. The package is pinned to `sha256:ca7e5d0bc440b70d2f3d01869a7aa7bae96cf5290bdf4fd70391dd5a4b9b79c0` and keeps the same persistent `/data` volume. No pricing formulas or database schema are changed by this release.
...
The released package uses a versioned, immutable-digest-pinned GHCR image. Release CI requires the exact pinned image to be anonymously readable because umbrelOS Community App installation does not use GitHub registry credentials. Package manifest/image version alignment and digest pinning remain permanent release gates.
